...
describe('Blog app', () => {
  ...
  describe('Login', () => {
    test('succeeds with correct credentials', async ({ page }) => {
      await loginWith(page, 'sillimies', 'salainen');
      await expect(page.getByText('Silli Mies is logged in')).toBeVisible();
    });

  test("fails with wrong credentials", async ({ page }) => {
    await page.fill('[data-testid="username"]', "sillimies");
  ...
  describe('When logged in', () => {
    ...
    test('a new blog can be created', async ({ page }) => {
      const blog = { title: 'created by playwright', author: 'test author', url: 'test-url' };
      await createBlog(page, blog);
      await expect(page.getByText('created by playwright - test author')).toBeVisible();
    });

    test('a blog can be liked', async ({ page }) => {
      const blog = { title: 'new blog', author: 'test author', url: 'test-url' };
      await createBlog(page, blog);
    ...
    test("only the user who added the blog can delete it", async ({
      ...
      await loginWith(page, 'Silli', 'sala');
      await page.getByRole('button', { name: 'view' }).click();
    ...
    test('blogs are arranged in the order according to the likes', async ({ page, request }) => {
      ...
      const token = await page.evaluate(() => localStorage.getItem('loggedUser')).then((userString) => userString && JSON.parse(userString).token);
      ...
      const sortedBlogTitles = blogs.sort((a, b) => b.likes - a.likes).map((b) => b.title);
